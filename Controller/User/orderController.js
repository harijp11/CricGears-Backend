...
async function cancelOrder(req,res){
    ...
    return res
    .status(200)
    .json({success:true,message:"Your order has been successfully cancelled"})
    }catch(err){
        console.log(err)
...
function generateTableRow(pdfDoc, y, item, size, quantity, price, total) {
  ...
  function generatePaymentSummary(pdfDoc, orderData) {
    ...
    generateSummaryRow(
      ...
      subtotalPosition,
      "Subtotal:",
      formatCurrency(orderData.totalAmount + (orderData.totalDiscount - orderData.couponDiscount))
    );
    generateSummaryRow(
      pdfDoc,
      subtotalPosition + 20,
