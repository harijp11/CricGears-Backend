...
const checkSizeAvailable = async (req, res) => {
  try {
    ...
    if (!Array.isArray(cartItems)) {
      return res
        .status(400)
        .json({ message: "Invalid cart items", success: false });
    }
    ...
    res.status(200).json({ success: true, message: "All items are available" });
  } catch (err) {
    console.log(err);
...
async function lockingQuantity(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "All items locked successfully" });
  } catch (err) {
...
async function unlockQuantities(req, res) {
  try {
    ...
    if (!Array.isArray(cartItems) || cartItems.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No items to unlock" });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Items unlocked successfully" });
  } catch (err) {
